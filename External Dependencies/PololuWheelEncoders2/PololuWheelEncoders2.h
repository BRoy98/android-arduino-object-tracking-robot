...
class PololuWheelEncoders
{
  public:
	 ...
	 */
	static void init(unsigned char rightA, unsigned char rightB, unsigned char leftA, unsigned char leftB);
	 ...
	 * the functions below.
	 */
	static long getCountsRight();
	static long getCountsLeft();

	/*
	 * These functions get the number of counts and reset the stored
	 * number to zero.
	 */
	static long getCountsAndResetRight();
	static long getCountsAndResetLeft();
	 ...
	 * flag.
	 */
	static unsigned char checkErrorRight();
	static unsigned char checkErrorLeft();
};
